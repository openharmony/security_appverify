...
{
    if (!CheckPkcs7SignedDataIsValid(pkcs7Context.p7)) {
        HAPVERIFY_LOG_ERROR("p7 type is invalid signed_data_pkcs7");
        return false;
    }

    if (!VerifyPkcs7SignedData(pkcs7Context)) {
        HAPVERIFY_LOG_ERROR("verify p7 error");
        return false;
    }
    return true;
}
...
bool HapVerifyOpensslUtils::VerifyPkcs7SignedData(Pkcs7Context& pkcs7Context)
{
    ...
    if (p7Bio == nullptr) {
        HAPVERIFY_LOG_ERROR("get p7bio error");
        GetOpensslErrorMessage();
        return false;
    }
    ...
    for (int32_t i = 0; i < signCount; i++) {
        if (!VerifySignInfo(signerInfoStack, p7Bio, i, pkcs7Context)) {
            HAPVERIFY_LOG_ERROR("Verify %{public}dst signInfo failed", i);
            BIO_free_all(p7Bio);
            return false;
        }
    }
...
bool HapVerifyOpensslUtils::VerifySignInfo(STACK_OF(PKCS7_SIGNER_INFO)* signerInfoStack,
    ...
    if (signInfo == nullptr) {
        HAPVERIFY_LOG_ERROR("signInfo %{public}dst is nullptr", signInfoNum);
        return false;
    }
    /* GET X509 certificate */
    ...
    if (isShaWithRsaPss) {
        ...
        if (pkey == nullptr) {
            HAPVERIFY_LOG_ERROR("signInfo %{public}dst X509_get_pubkey failed", signInfoNum);
            return false;
        }
        HAPVERIFY_LOG_DEBUG("use RSA/pss");
        if (!VerifyShaWithRsaPss(signInfo, p7Bio, pkey, isShaWithRsaPss)) {
            HAPVERIFY_LOG_ERROR("VerifyShaWithRsaPss %{public}dst signInfo failed", signInfoNum);
            return false;
        }
    } else {
        if (PKCS7_signatureVerify(p7Bio, pkcs7Context.p7, signInfo, cert) <= 0) {
            HAPVERIFY_LOG_ERROR("PKCS7_signatureVerify %{public}dst signInfo failed", signInfoNum);
            GetOpensslErrorMessage();
            return false;
        }
    }
    ...
    BIO* p7Bio, EVP_PKEY* pkey, bool isPss)
{
    if (signInfo->digest_alg == nullptr) {
        HAPVERIFY_LOG_ERROR("signInfo->digest_alg is nullptr");
        return false;
    }
    ...
    if (!EVP_MD_CTX_copy_ex(mdCtxTmp, mdCtx)) {
        HAPVERIFY_LOG_ERROR("EVP_MD_CTX_copy_ex failed");
        EVP_MD_CTX_free(mdCtxTmp);
        return false;
    }
    if (!VerifyPkcs7AuthAttributes(signInfo, mdCtxTmp, mdType)) {
        HAPVERIFY_LOG_ERROR("VerifyPkcs7AuthAttributes failed");
        EVP_MD_CTX_free(mdCtxTmp);
        return false;
    }
    ...
    if (EVP_DigestFinal_ex(mdCtxTmp, digest, &digestLen) <= 0) {
        HAPVERIFY_LOG_ERROR("Digest content failed");
        GetOpensslErrorMessage();
        EVP_MD_CTX_free(mdCtxTmp);
        return false;
    }
    ...
    if (!VerifyShaWithRsaPss(signInfo, pkey, isPss, digest, digestLen)) {
        HAPVERIFY_LOG_ERROR("VerifyShaWithRsaPss failed");
        GetOpensslErrorMessage();
        return false;
    }
    return true;
...
const EVP_MD_CTX* HapVerifyOpensslUtils::FindMdCtxInBio(BIO* p7Bio, int32_t mdType)
{
    ...
    while (p7Bio) {
        ...
        if (mdCtx == nullptr) {
            HAPVERIFY_LOG_ERROR("Get null from bio");
            return nullptr;
        }
    ...
    const PKCS7_SIGNER_INFO* signInfo, EVP_MD_CTX* mdCtx, int32_t mdType)
{
    ...
    if ((authAttributes != nullptr) && (sk_X509_ATTRIBUTE_num(authAttributes) != 0)) {
        ...
        if (!AsnStringCmp(digestInAttribute, digest, static_cast<int>(digestLen))) {
            HAPVERIFY_LOG_ERROR("AsnStringCmp failed");
            return false;
        }

        if (!EVP_VerifyInit_ex(mdCtx, EVP_get_digestbynid(mdType), nullptr)) {
            HAPVERIFY_LOG_ERROR("EVP_VerifyInit_ex failed");
            GetOpensslErrorMessage();
            return false;
        }
        ...
        if (attributesLen <= 0 || attributesData == nullptr) {
            HAPVERIFY_LOG_ERROR("ASN1_item_i2d failed");
            GetOpensslErrorMessage();
            return false;
        }
        if (!EVP_VerifyUpdate(mdCtx, attributesData, attributesLen)) {
            HAPVERIFY_LOG_ERROR("EVP_VerifyUpdate failed");
            GetOpensslErrorMessage();
...
{
    if (asnStr == nullptr) {
        HAPVERIFY_LOG_ERROR("asnStr is nullptr");
        return false;
    }
    if (asnStr->data == nullptr) {
        HAPVERIFY_LOG_ERROR("asnStr->data is nullptr");
        return false;
    }
    if (data == nullptr) {
        HAPVERIFY_LOG_ERROR("data is nullptr");
        return false;
    }
    if (asnStr->length != len) {
        HAPVERIFY_LOG_ERROR("asnStr->length: %{public}d is not equal to len: %{public}d", asnStr->length, len);
        return false;
    }
    for (int32_t i = 0; i < len; i++) {
        if (asnStr->data[i] != data[i]) {
            HAPVERIFY_LOG_ERROR("%{public}dst data is not equal", i);
            return false;
        }
    }
    return true;
    ...
    const unsigned char digest[], uint32_t digestLen)
{
    ...
    if (pkeyCtx == nullptr) {
        HAPVERIFY_LOG_ERROR("EVP_PKEY_CTX_new failed");
        GetOpensslErrorMessage();
        return false;
    }
    if (EVP_PKEY_verify_init(pkeyCtx) <= 0) {
        HAPVERIFY_LOG_ERROR("EVP_PKEY_verify_init failed");
        GetOpensslErrorMessage();
        EVP_PKEY_CTX_free(pkeyCtx);
        return false;
    }
    if (signInfo->digest_alg == nullptr || signInfo->enc_digest == nullptr) {
        HAPVERIFY_LOG_ERROR("no digest_alg or enc_digest in signInfo");
        EVP_PKEY_CTX_free(pkeyCtx);
        return false;
    }
    int32_t mdType = OBJ_obj2nid(signInfo->digest_alg->algorithm);
    if ((isPss && EVP_PKEY_CTX_set_rsa_padding(pkeyCtx, RSA_PKCS1_PSS_PADDING) <= 0) ||
        ...
        HAPVERIFY_LOG_ERROR("set rsa_padding or signature_md failed");
        GetOpensslErrorMessage();
        EVP_PKEY_CTX_free(pkeyCtx);
        return false;
    }
    if (EVP_PKEY_verify(pkeyCtx, signInfo->enc_digest->data, signInfo->enc_digest->length, digest, digestLen) <= 0) {
        HAPVERIFY_LOG_ERROR("EVP_PKEY_verify failed");
        GetOpensslErrorMessage();
        EVP_PKEY_CTX_free(pkeyCtx);
        return false;
    }
    ...
    std::vector<std::string>& SignatureVec)
{
    for (uint32_t i = 0; i < signCertChain.size(); i++) {
        if (!GetPublickeyFromCertificate(signCertChain[i], SignatureVec)) {
            HAPVERIFY_LOG_ERROR("%{public}ust Get Publickey failed", i);
            return false;
        }
    }
    ...
    std::vector<std::string>& SignatureVec)
{
    for (uint32_t i = 0; i < signCertChain.size(); i++) {
        if (!GetDerCert(signCertChain[i], SignatureVec)) {
            HAPVERIFY_LOG_ERROR("%{public}ust GetDerCert failed", i);
            return false;
        }
    }
    return !SignatureVec.empty();
...
bool HapVerifyOpensslUtils::GetPublickeyFromCertificate(const X509* ptrX509, std::vector<std::string>& publicKeyVec)
{
    ...
    if (!HapCertVerifyOpensslUtils::GetPublickeyBase64(ptrX509, publicKey)) {
        HAPVERIFY_LOG_ERROR("GetPublickeyBase64 Failed");
        return false;
    }
...
bool HapVerifyOpensslUtils::GetContentInfo(const PKCS7* p7ContentInfo, HapByteBuffer& content)
{
    if ((p7ContentInfo == nullptr) || !PKCS7_type_is_data(p7ContentInfo)) {
        HAPVERIFY_LOG_ERROR("p7ContentInfo is invalid");
        return false;
    }

    ASN1_OCTET_STRING* strContentInfo = p7ContentInfo->d.data;
    if (strContentInfo == nullptr) {
        HAPVERIFY_LOG_ERROR("strContentInfo is invalid");
        return false;
    }
...
bool HapVerifyOpensslUtils::CheckDigestParameter(const DigestParameter& digestParameter)
{
    if (digestParameter.md == nullptr) {
        HAPVERIFY_LOG_ERROR("md is nullptr");
        return false;
    }
...
bool HapVerifyOpensslUtils::DigestInit(const DigestParameter& digestParameter)
{
    ...
    if (EVP_DigestInit(digestParameter.ptrCtx, digestParameter.md) <= 0) {
        GetOpensslErrorMessage();
        HAPVERIFY_LOG_ERROR("EVP_DigestInit failed");
        return false;
    }
    return true;
}
...
{
    if (content == nullptr) {
        HAPVERIFY_LOG_ERROR("content is nullptr");
        return false;
    }
    if (!CheckDigestParameter(digestParameter)) {
        return false;
    ...
    if (EVP_DigestUpdate(digestParameter.ptrCtx, content, len) <= 0) {
        GetOpensslErrorMessage();
        HAPVERIFY_LOG_ERROR("EVP_DigestUpdate chunk failed");
        return false;
    }
    return true;
}
...
int32_t HapVerifyOpensslUtils::GetDigest(const DigestParameter& digestParameter, unsigned char (&out)[EVP_MAX_MD_SIZE])
{
    ...
    if (EVP_DigestFinal(digestParameter.ptrCtx, out, &outLen) <= 0) {
        GetOpensslErrorMessage();
        HAPVERIFY_LOG_ERROR("EVP_DigestFinal failed");
        outLen = 0;
    }
    return outLen;
}
    ...
    const DigestParameter& digestParameter, unsigned char (&out)[EVP_MAX_MD_SIZE])
{
    ...
    uint32_t outLen = 0;
    if (digestParameter.md == nullptr) {
        HAPVERIFY_LOG_ERROR("md is nullprt");
        return outLen;
    }
    if (digestParameter.ptrCtx == nullptr) {
        HAPVERIFY_LOG_ERROR("ptrCtx is nullprt");
        return outLen;
    }
...
void HapVerifyOpensslUtils::GetOpensslErrorMessage()
{
    ...
    while ((retOpenssl = ERR_get_error()) != 0) {
        ...
        HAPVERIFY_LOG_ERROR("openssl err: %{public}lu, message: %{public}s", retOpenssl, errOpenssl);
    }
}
...
int32_t HapVerifyOpensslUtils::GetDigestAlgorithmId(int32_t signAlgorithm)
{
    switch (signAlgorithm) {
        ...
        case ALGORITHM_SHA512_WITH_DSA:
            return NID_sha512;
        default:
            HAPVERIFY_LOG_ERROR("signAlgorithm: %{public}d error", signAlgorithm);
            return NID_undef;
    }
}
} // namespace Verify
